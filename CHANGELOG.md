...
**修正 `b(msg)` 语义反转 + `q1.B` 静态开关直置 + `iu.l1` 责任判定日志。**

### 根因
- `lo5.f/mq5.f.b(msg)` 返回 `true` = 已过期不可播，v2.11.0 误读为"不可播"反向放行（`false->true`），把未过期消息也改了一遍（无害但刷屏且逻辑错）。v2.11.1 改为仅 `true->false`（过期误判纠正），正常 `false` 不动。
- `q1.B` 静态总开关：画廊构造时 `B = gate.a()` 赋值一次，若构造时序早于 hook 即恒 false；模块启动时直接置 `true`，不等构造赋值。
- `iu.l1.b(xml)` trace：K() 最后一步验消息 XML 附件；若门控全通仍无按钮，日志打出 `iu.l1.b(xml)->false` 即消息体本身无实况数据（历史消息落库时即缺失，需对方重发新消息验证）。

### 验证
- 3141 无线机：`b(msgId) true->false` + `iu.l1.b(xml)->false` 链完整，门控侧闭环；按钮是否出现取决于消息体（新消息验证中）。
